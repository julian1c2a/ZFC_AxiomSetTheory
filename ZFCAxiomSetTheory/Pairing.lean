  ...
  namespace PairingAxiom
    ...
    theorem fst_of_ordered_pair (x y : U) :
      ...
      have h_inter_w : (⋂ ⟨x, y⟩) = {x} := by
        -- Para probar la igualdad de conjuntos, usamos extensionalidad: probamos que tienen los mismos elementos.
        apply ExtSet
        intro z
        -- El objetivo es: z ∈ ⋂ ⟨x, y⟩ ↔ z ∈ {x}

        -- Primero, necesitamos la hipótesis de que el par ordenado no es vacío para usar la especificación de la intersección.
        have h_nonempty : (⟨x, y⟩ : U) ≠ ∅ := OrderedPair_is_nonempty x y
        have h_exists_elem : ∃ elem, elem ∈ (⟨x, y⟩ : U) := (nonempty_iff_exists_mem _).mp h_nonempty

        -- Ahora podemos usar la especificación de la Intersección.
        rw [Intersection_is_specified (⟨x, y⟩) z h_nonempty h_exists_elem]
        -- El objetivo se convierte en: (∀ w, w ∈ ⟨x, y⟩ → z ∈ w) ↔ z ∈ {x}

        -- Usamos la especificación de singleton y par ordenado.
        rw [(OrderedPair_is_specified x y z).mpr (Or.inl rfl), Singleton_is_specified]
        -- El objetivo ahora es: (∀ w, (w = {x} ∨ w = {x, y}) → z ∈ w) ↔ z = x
        constructor
        · -- Dirección: ->
          intro h_all_mem
          -- h_all_mem : ∀ w, (w = {x} ∨ w = {x, y}) → z ∈ w
          -- Podemos aplicar esto a w = {x}.
          have hz_in_singleton : z ∈ {x} := h_all_mem {x} (Or.inl rfl)
          -- De z ∈ {x}, obtenemos z = x por la definición de singleton.
          exact (Singleton_is_specified x z).mp hz_in_singleton
        · -- Dirección: <-
          intro h_z_eq_x
          -- h_z_eq_x : z = x
          -- Necesitamos probar: ∀ w, (w = {x} ∨ w = {x, y}) → z ∈ w
          intro w h_w_cases
          -- Tenemos dos casos para w.
          cases h_w_cases with
          | inl h_w_eq_singleton =>
            -- Caso 1: w = {x}. Necesitamos probar z ∈ {x}.
            rw [h_w_eq_singleton, Singleton_is_specified]
            exact h_z_eq_x
          | inr h_w_eq_pair =>
            -- Caso 2: w = {x, y}. Necesitamos probar z ∈ {x, y}.
            rw [h_w_eq_pair, PairSet_is_specified, h_z_eq_x]
            -- El objetivo es x = x ∨ x = y, lo cual es cierto.
            exact Or.inl rfl
